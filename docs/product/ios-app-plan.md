...
| 里程碑 | 窗口 | 要点 |
|---|---|---|
| **Cust-M0** | 现在 | 独立仓、SwiftUI 壳、Staging/Mock API、服务目录、发起请求、我的任务只读 | **完成（本地）** |
| **Cust-M1** | 随后 | 配送请求闭环 + 状态时间线 + 取消 + 中文 UI | **完成（本地）** |
| Cust-M2 | 随后 | 清洁 / 酒店送物体验细化（表单/文案/隐私） | 待做 |
| Cust-M3 | Pilot 前 | 登录（OIDC）、推送、站点扫码/选桌 | 待做 |
| Cust-GA | 2027-02 Pilot | 与 Pilot 证据包对齐 | 待做 |
